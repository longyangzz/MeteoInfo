package com.dclw.ncforecast.common;
...
public class Core {
    ...
    public static boolean IsPtInPoly(Point2D point, List<Point2D> pts) {
        ...
        for (int i = 1; i <= N; ++i) {// check all rays
            ...
            if (p.getX() < Math.min(p1.getX(), p2.getX()) || p.getX() > Math.max(p1.getX(), p2.getX())) {// ray is
                // interests
                p1 = p2;
            ...
            if (p.getX() > Math.min(p1.getX(), p2.getX()) && p.getX() < Math.max(p1.getX(), p2.getX())) {// ray is
                ...
                if (p.getY() <= Math.max(p1.getY(), p2.getY())) {// x is before of ray
                    if (p1.getX() == p2.getX() && p.getY() >= Math.min(p1.getY(), p2.getY())) {// overlies on a
                        // horizontal ray
                ...
                if (p.getX() == p2.getX() && p.getY() <= p2.getY()) {// p crossing over p2
                    ...
                    if (p.getX() >= Math.min(p1.getX(), p3.getX()) && p.getX() <= Math.max(p1.getX(), p3.getX())) {// p.x
                        // p3.x
                        ++intersectCount;
                    } else {
                        intersectCount += 2;
